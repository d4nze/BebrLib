...
void bebr::system::Window::setTitle(const char* title)
{
    ...
    if (m_hWnd) { SetWindowText(m_hWnd, m_wTitle.c_str()); }
}

void bebr::system::Window::setMaxFrameRate( unsigned int maxFPS ) { m_fpsLimit = maxFPS; }
...
bebr::math::Vector2i bebr::system::Window::getSize() const { return { m_width, m_height }; }

const char* bebr::system::Window::getTitle() const { return m_title; }

bool bebr::system::Window::isOpen() const { return IsWindowVisible(m_hWnd); }

unsigned int bebr::system::Window::getMaxFrameRate() const { return m_fpsLimit; }

bool bebr::system::Window::getFullscreen() const
{
    DWORD style = GetWindowLong(m_hWnd, GWL_STYLE);
...
void bebr::system::Window::close() const
{
    if (m_hWnd) { PostMessage(m_hWnd, WM_CLOSE, 0, 0); }
}

void bebr::system::Window::makeCurrent() const { wglMakeCurrent(m_hDC, m_hRC); }

void bebr::system::Window::swapBuffers() const { SwapBuffers(m_hDC); }
